    ...
    function CMB2Conditional() {
        $('[data-conditional-id]').each( (i, el) => { 
            let condName    = el.dataset.conditionalId,
                ...
                var condParent  = el.closest('.cmb-row')
                console.log($(condParent).hasClass('postbox'));
                if( $(condParent).hasClass('postbox')){ 
                    condParent = $(condParent).parent().closest('.cmb-row')[0] 
                    console.log(condParent)
                }
                let inGroup     = condParent.classList.contains('cmb-repeat-group-field');
